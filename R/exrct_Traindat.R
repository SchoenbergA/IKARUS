...
#' @description Extracts values from a RasterStack of predictor variables by TrainingPolygons
...
#' @return Returns a dataframe with the Values of each Rasterlayer per pixel for the TrainingPolygons.
#' Additionally adds a column of the respectiv class informations.
#' @details This function is used to extract TrainingData from a Raster Stack. This TraingDataset is used for IKARUS::BestPredFFS and IKARUS::RFclass.

#' * classCol - the column with infromation about the class of a polygon. Supports either char or numeric values for classes (eg 1,2,3 or "tree","stone","grass")
#' * predictor selection - specific predictors can be selected by using [[]] in 'predStk. E.g. predStk= x[[1:4]].
...
#' ### check column names
#' names(lau_tP)
#' # -> lau_tP has both character and numeric class informations
